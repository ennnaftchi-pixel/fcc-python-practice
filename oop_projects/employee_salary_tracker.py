...
Tracks employees, levels, promotions, and salaries.
Part of freeCodeCamp Python OOP practice projects.
"""

class Employee:
    """Represents an employee with a name, level, and salary.
    Supports promotions and salary updates with validation.
    """
    ...
    def __init__(self, name, level):
        if not (isinstance(name, str) and isinstance(level, str)):
            raise TypeError("'name' and 'level' must be strings.")
    ...
    def level(self, new_level):
        ...
        if Employee._base_salaries[new_level] < Employee._base_salaries[self.level]:
            raise ValueError(f"Cannot change to a lower level.")
